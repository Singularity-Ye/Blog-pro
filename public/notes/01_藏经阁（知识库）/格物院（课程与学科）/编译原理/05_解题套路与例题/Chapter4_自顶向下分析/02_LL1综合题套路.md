...
$\text{FOLLOW}(A)$ 是指在某些句型中紧跟在非终结符 $A$ 右侧的终结符集合（包含结束标记 $\$$）。
* **初始条件**：将结束符 $\$$ 放入开始符号 $S$ 的 FOLLOW 集中：$\{\$\} \subseteq \text{FOLLOW}(S)$。
...
2. 若 $\varepsilon \in \text{FIRST}(\alpha)$（即 $\alpha \Rightarrow^* \varepsilon$），则对所有的 $b \in \text{FOLLOW}(A)$（包含 $\$$），填入：
