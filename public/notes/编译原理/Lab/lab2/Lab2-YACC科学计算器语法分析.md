...
| 维度       | 说明                                                                                          |
| -------- | ------------------------------------------------------------------------------------------- |
| **前置**   | [[编译原理：自底向上语法分析\|Lab 1]] 已完成词法分析——Flex 识别 token、输出 token 流                    |
...
利用 [[Yacc与LALR(1)|Yacc 的 %union 机制]]：
...
不是让 Bison 别报错就完了。是要**故意写出有歧义的文法**，理解冲突在哪里、为什么产生、如何消解。这正是 [[LR系列对比总览|LALR(1) 的理论知识]]在工程中的直接应用。
...
Yacc/Bison 底层用的是 [[LR系列对比总览|LALR(1) 算法]]。当 LALR(1) 分析表中出现 shift/reduce 冲突时，Yacc 的默认行为是**优先 shift**（这就是为什么悬挂 else 能自动处理）。但对于算术表达式，默认的 shift 优先会导致错误的结合性（例如 `a - b - c` 变成 `a - (b - c)`），所以必须显式声明 `%left` 来覆盖默认行为。

这就是 [[Yacc与LALR(1)|Yacc 歧义消除机制]]在表达式文法中的具体形式。
...
|------|------|
| LALR(1) 理论 | [[LR系列对比总览]] |
| Yacc 语法规则与语义动作 | [[Yacc与LALR(1)]] |
| shift/reduce 冲突 | [[移进-归约冲突]] |
| reduce/reduce 冲突 | [[归约-归约冲突]] |
| 综合属性（`$$`/`$n` 的理论基础） | [[综合属性]] |
| S-属性文法的天然适应性 | [[S-属性文法]] |
| 属性文法三元组 | [[属性文法]] |
| ACTION/GOTO 表 | [[Action 表]] / [[Goto 表]] |
| 项目集规范族 | [[项目集规范族]] |
| Lab 1 词法分析 | [[编译原理：自底向上语法分析]] |
| 编译原理阶段总结 | [[编译原理阶段总结]] |
