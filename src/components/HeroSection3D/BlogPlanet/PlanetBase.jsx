...
const fragmentShader = `
  ...
  float getCoastMask(vec3 normal) {
    float mask = 0.0;

    for (int i = 0; i < ${MAX_LAND_CENTERS}; i++) {
      float dotValue = dot(normal, uLandCenters[i]);
      float localMask = smoothstep(0.82 - uLandWidths[i], 0.958, dotValue);
  ...
  void main() {
    ...
    float seamSafe = smoothstep(0.012, 0.042, vUv.x) * smoothstep(0.012, 0.042, 1.0 - vUv.x);
    float seamMask = 1.0 - seamSafe;
    vec3 uvAlbedo = sampleUv(uAlbedoMap, vUv, 2.15, flowA);
    vec3 triAlbedo = sampleTriplanar(uAlbedoMap, normal, 2.15, flowA);
    float uvNoiseA = sampleUvR(uNoiseMap, vUv, 2.35, flowA);
    float uvNoiseB = sampleUvR(uNoiseMap, vUv, 5.1, flowB);
    ...
    float coast = getCoastMask(normal);
    float openSea = 1.0 - coast;
    ...
    ocean = mix(ocean, shallow, smoothstep(0.14, 0.66, coast) * 0.78);
    ocean = mix(ocean, coastColor, smoothstep(0.58, 0.98, coast) * 0.46);

    float uvFoam = sampleUvR(uFoamMap, vUv, 3.5, flowFoam);
    ...
    float foam = smoothstep(0.74, 0.94, foamTexture) * smoothstep(0.34, 0.96, coast);
    float glint = ripple * (0.075 + coast * 0.05);
